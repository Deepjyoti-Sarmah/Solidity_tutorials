...
/*
  ...
  * 
  *So if the Chainlink network explodes and you have lot of money locked in the protocol...to bad.
  *
  */
library OracleLib {
  ...
  function staleCheckLatestRoundData(AggregatorV3Interface priceFeed) public view returns(uint80, int256, uint256, uint80) {
    ...
    uint256 secondsScince = block.timestamp - updatedAt;
    if (secondsScince > TIMEOUT) {
